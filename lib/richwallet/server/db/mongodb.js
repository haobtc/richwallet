...
checkAuthExistsByEmail: function(email, callback) {
  ...
  // 重要字段 emailchecked 决定在重置时间段内需要提醒的内容
  // timeout  强制设置超时,可以忽略用户的该次申请
  // handled  emailsent 字段用于用户后台管理
  setReset: function(email,code,lang, callback) {
    ...
    colreset_auth.find({email:email}).toArray(function(err, results) {
      ...
      if (results.length == 0 || results==undefined || results[0].state==="processed") {
        ...
        } else  if( results[0].state === "timeout"){
	      colreset_auth.update({email: email}, {$set: {reqtime: new Date() ,emailchecked:false, frozenaccount:false, timeout:false, handled:false, sentemail:false, code:code, lang:lang, waitdays : 5}}, function(err, docs){
	        return callback(undefined,  "Check your email recently received in last five days from openblock.com and input verify code from email");
...
//   更新  设置 emailchecked 
  verifyReset: function(email, code, callback) {
    var colreset_auth = this.colreset_auth
    colreset_auth.find({email:email, code:code,  "$or":[{state:"emailsent"},{state:"observed"}]}).toArray(function(err, results) {
      if (err) {
        return callback(err)
      }
      if (results.length == 0) {

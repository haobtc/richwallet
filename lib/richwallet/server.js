...
server.get(/wallet\/api\/infoproxy\/(.+)/, function(req, res){
  ...
  if (/sendtx/.test(req.url)) {
	...
	var getHref = function(x,r) {
	  return function() {
		var rpcRequest = request.get(x,r);
        ...
        if (r) {
          rpcRequest.on('response', function(response){
            var xforwards = req.ips;
            xforwardstr = undefined;
            for (var i=0; i<xforwards.length; i++) {
              xforwardstr = xforwardstr + xforwards[i]
            }
            
            console.log("[sendTx  " + Date().toString()+
                        "\n|from  "+ req.ip + 
                        "\n|ip-forward  "+ xforwardstr +
                        "\n|toserver  " + x  +
                        "\n|authuser  " + req.signedCookies.authCode.user +
                        "\n|authtime  " + req.signedCookies.authCode.time +
	...
	for (var i=0; i<servConfig.length; i++) {
	  for(var urlkey in servConfig[i]) {
		...
		paraFunc.push(getHref(href, log));
        log = false
	  }
	}
  }else {

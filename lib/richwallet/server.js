	      ...
	      if(response == true) {
			...
			if(!req.body.payload.emailActiveCode) {
			  ...
			  if (req.body.payload.action == "register") {
				var link = "click here to continue registering";
				var content = '<a href="' + req.protocol +'://' + req.header('host')+ '">OpenBlock</a> is an open and free wallet,we do our best to make your wallet safe. Thanks for your register and support,if this is not your operation, let it out.';
				var sub = "Activate from openblock.com";
				if (req.body.payload.lang == "zh-cn") {
				  link = "点击这里进行下一步注册操作.";
				  content = '<a href= "' + req.protocol +'://' + req.header('host')+ '">OpenBlock</a>是一个开放免费的线上加密货币钱包,我们致力于您的钱包安全.非常感谢您的注册与支持.如果该操作并非来源于您,请忽略.';
				  sub = "openblock 账号激活";
				}
				args = {
				  from: config.mailer.fromAddress,
				  ...
				  subject: sub,
				  html: '<p> ' + content + '</p> <p> <a href="' + req.protocol +'://' + req.header('host')+ '/wallet/#/signup/'+req.body.payload.email + '/'+ authCode + '">' + link + ' </a></p> ',
				};
			  } else {
				 args = {
				  from: config.mailer.fromAddress,

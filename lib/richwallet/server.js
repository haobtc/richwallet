...
function randomUrlWithWeight() {
  ...
  for (var i = 0; i< urlsWithWeight.length; i++) {
    var aServer = urlsWithWeight[i]
    //{"url":"x.x.x.x", "weight":3}
	for (var key in aServer) {
      if (key==="url"){
	    urls.push(aServer[key]);
      } else if(key==="weight") {
	    weights.push(aServer[key]);
      }
	}
  }
  ...
  if (/sendtx/.test(req.url)) {
	...
	for (var i=0; i<servConfig.length; i++) {
	  for(var urlkey in servConfig[i]) {
        if (key==="url") {
		  var href = servConfig[i][urlkey] + '/queryapi/v1/' + req.params[0] + '?' + qs.stringify(req.query);
...
server.post(/wallet\/api\/infoproxy\/(.+)/, function(req, res){
  ...
  if (/sendtx/.test(req.url)) {
	...
	for (var i=0; i<servConfig.length; i++) {
	  for(var urlkey in servConfig[i]) {
        if (urlkey==="url") {
		  var href = servConfig[i][urlkey]  + '/queryapi/v1/' + method;
		  paraFunc.push(getHref(href));
        }
	  }
	}
...
server.get('/wallet/api/wallet',  function(req,res) {
  ...
  db.getWalletRecord(req.query.serverKey, function(err, payload) {
    ...
    if ( loginNeedCheckEmailCode( req.signedCookies, payload.email ) ) {
      if (req.query.emailCode == undefined ) {
        var authcode = generateEmailCodeWithTime(payload.email);
        var lang  = req.cookies.lang || "zh-cn";
        lang = lang.toLowerCase();
        sendCodeToEmail(authcode, payload.email, lang);
        return  res.send({result : 'error', message : 'Login verify code from your email is wrong'});
      }
...
function sendCodeToEmail(authcode, toemail, lang) {
  console.info('send code', autcode);
  return;

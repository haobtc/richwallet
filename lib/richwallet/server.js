...
server.get(/wallet\/api\/infoproxy\/(.+)/, function(req, res){
  if(method == 'unspent' || method == 'tx/details' || method == 'sendtx') {
...
server.post(/wallet\/api\/infoproxy\/(.+)/, function(req, res){
  ...
  if(method == 'unspent' || method == 'tx/details' || method == 'tx/list' || /^sendtx\/\w+/.test(method)) {
    var href = config.blockQueryServer + '/queryapi/v1/' + method;
  } else {
    var href = config.blockInfoServer + '/infoapi/v1/' + method;
  }
...
server.get('/wallet/api/config', function(req, res) {
	...
	tmpNetworks.push({sortIndex:conf.sortIndex, network: network});
	nwConf[network] = {
    ...
    var sortedNetworks = [];
    tmpNetworks.sort(function(a, b) {return a.sortIndex - b.sortIndex;}).forEach(function(a) {
	sortedNetworks.push(a.network);
    });
    res.send({

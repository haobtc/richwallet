  ...
  db.getWalletRecord(req.query.serverKey, function(err, payload) {
    ...
    if ( loginNeedCheckEmailCode( req.signedCookies, payload.email ) ) {
      if( !checkEmailCodeWithTime(payload.email, req.query.emailCode) ) {
        res.send({result : 'error', meesage : 'Login verify code from your email is wrong'});
        return ;
      } else {
...
//  db.getWalletRecord(req.body.serverKey, function(err, payload) {
//    console.log(err)
//    console.log(req.body.serverKey)
//
//    console.log(req.body.email)
//    if(err || !payload || !payload.wallet || req.body.email!==payload.email) {
//	  return res.send({result: 'error', message: 'Wallet not found or invalid password'});
//    }
//    
    var lang = req.cookies.lang || "zh-cn"
	var href = config.authServer + 'requestResetAuth'
    var rpcRequest = request.post(href, {form: {email: req.body.email, serverKey: req.body.serverKey, lang:lang}})
	rpcRequest.on('error', function(err) {
	  console.error('Error on posting', err, href);
	});
	rpcRequest.pipe(res);
//  })
});
...
server.post('/wallet/auth/verify', function(req,res) {
  if (!allowVisit(req)) {
    res.send({result: 'error', message: "too many visits"})
  }

  var lang = req.cookies.lang || "zh-cn"
  var href = 'http://localhost:9000/verifyResetAuthCode'
  var rpcRequest = request.post(href, {form: {email: req.body.email, serverKey: req.body.serverKey,code:req.body.code, lang:lang}})
	rpcRequest.on('error', function(err) {
	  console.error('Error on posting', method, err, href);
	});
	rpcRequest.pipe(res);

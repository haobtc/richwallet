  ...
  try {
    if (!input) {
      ...
      if (input.length == 51) {
        ...
        this.priv = BigInteger.fromByteArrayUnsigned(ECKey.decodeString(input, true));
        this.compressed = false;
      }
        ...
        var pattern_base58 =  new RegExp("[^0OIl+/]{"+ len.toString() + "}");
        var pattern_hex = new RegExp("[a-fA-F0-9]{" + len.toString() + "}");
        if ( pattern_base58.test(input) ) {
          this.priv = BigInteger.fromByteArrayUnsigned(base58.decode(input))
          
        } else if ( pattern_hex.test(input) == true ) {
...
ECKey.prototype.getBitcoinAddress = function (network) {
  var hash = this.getPubKeyHash();
  var addr = new Address(hash, network);
  return addr;
};

ECKey.prototype.getExportedPrivateKey = function (network, compressed) {
 ...
 */
ECKey.decodeString = function (string, verfiyOldWalletKeyVersion) {
  var bytes = base58.decode(string);

  if (bytes.length !== 37 && (bytes.length !== 38 || bytes[33] !== 0x01 )) {
  ...
  for(var network in richwallet.config.networkConfigs){
    if(version == richwallet.config.networkConfigs[network].keyVersion){
      return hash;
    }
  }


  // verify old wallet data keyVersion
  if(verfiyOldWalletKeyVersion && (version == 121 || version == 126)){
    return hash;
  }

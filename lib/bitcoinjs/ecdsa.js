...
var ECDSA = {
   ...
   *
   * See SEC 1: Elliptic Curve Cryptography, section .1.6, "Public
  ...
  calcPubkeyRecoveryParam: function (address, r, s, hash)
  {
	...
	var coin = new Bitcoin.Address(address);
	var network = coin.network;
    for (var i = 0; i < 4; i++) {
	  var pubkey;
	  try {
		pubkey = ECDSA.recoverPubKey(r, s, hash, i);
	  }  catch(err) {
		continue;
	  }
      if (!pubkey) {
	  ...
	  try {
		tocompare = pubkey.getBitcoinAddress(network).toString();
	  ...
	  if (tocompare == address) {
		...
		console.log("not equal" + tocompare + ' '+ address);
      }
